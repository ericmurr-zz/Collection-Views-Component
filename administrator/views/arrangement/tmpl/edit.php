...
<script type="text/javascript">
	...
	js('input:hidden.item_ids').each(function(){
		var name = js(this).attr('name');
		if(name.indexOf('item_idshidden')){
			js('#jform_item_ids option[value="'+js(this).val()+'"]').attr('selected',true);
		}
	});
	js("#jform_item_ids").trigger("liszt:updated");
    });

    Joomla.submitbutton = function(task)
    {
        if (task == 'arrangement.cancel') {
            Joomla.submitform(task, document.getElementById('arrangement-form'));
        }
        else {
            
            if (task != 'arrangement.cancel' && document.formvalidator.isValid(document.id('arrangement-form'))) {
                
	if(js('#jform_item_ids option:selected').length == 0){
		js("#jform_item_ids option[value=0]").attr('selected','selected');
	}
                Joomla.submitform(task, document.getElementById('arrangement-form'));
            }
            else {
...
<form action="<?php echo JRoute::_('index.php?option=com_collectionviews&layout=edit&id=' . (int) $this->item->id); ?>" method="post" enctype="multipart/form-data" name="adminForm" id="arrangement-form" class="form-validate">

    <div class="form-horizontal">
        <?php echo JHtml::_('bootstrap.startTabSet', 'myTab', array('active' => 'general')); ?>

        <?php echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_COLLECTIONVIEWS_TITLE_ARRANGEMENT', true)); ?>
        <div class="row-fluid">
			...
			<?php
				foreach((array)$this->item->item_ids as $value): 
					if(!is_array($value)):
						echo '<input type="hidden" class="item_ids" name="jform[item_idshidden]['.$value.']" value="'.$value.'" />';
					endif;
				endforeach;
